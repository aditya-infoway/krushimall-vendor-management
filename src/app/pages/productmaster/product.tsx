...
export default function ProductList() {
  ...
  const getProducts = async () => {
    try {
      ...
      const mappedData: ProductType[] = data.map((item: any) => ({
        ...
        subCategory: item.subCategory?.subCategoryName || item.subCategory?.name || "",
  ...
  const handleToggleStatus = async (id: number) => {
    ...
    try {
      ...
      await apiHelper.put(`/vendor-web/product/${id}`, { status: newStatus });
      setProducts((prev) =>
    ...
    <div className="relative min-h-screen space-y-6 p-4 pb-28 text-gray-900 md:p-6 dark:text-gray-100">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 md:flex-nowrap">
          ...
          <Button color="primary" onClick={handleAddProduct} className="whitespace-nowrap">
            Add Product
          </Button>
        </div>
      </div>
      ...
      <div className="dark:bg-dark-800 dark:border-dark-700 rounded-xl border border-gray-200 bg-white shadow-sm">
        <div className="overflow-x-auto">
          <Table hoverable className="w-full min-w-240 text-left [&_.table-th]:font-semibold">
                ...
                return (
                  ...
                  >
                    ...
                    <Td className="py-4 font-medium text-gray-900 dark:text-gray-400">
                      <span className="line-clamp-2 max-w-50">{item.productName}</span>
                    </Td>
                    <Td className="dark:text-dark-200 py-4 text-gray-600">{item.sku}</Td>
                    <Td className="dark:text-dark-200 py-4 text-gray-600">{item.category}</Td>
                    <Td className="dark:text-dark-200 py-4 text-gray-600">{item.brand}</Td>
                    <Td className="py-4">
